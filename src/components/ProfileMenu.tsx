...
import { config, UserAccountDict } from '@lib/constants'
...
export function UserMenu(): JSX.Element {
  ...
  return (
    <Box
      ...
      bg={['#ffffffe1', 'transparent', 'transparent']}
      ...
      {hasProfile && (
        <Menu closeOnSelect colorScheme="blue">
          <ButtonGroup
            ...
            pr={2}
          >
            <MenuButton rounded={['md', 'lg']} bg="blue.700" as={Button} pr={10} mr={-8}>
              {`${profile.ethBalance?.symbol} ${formatBalance(profile.ethBalance?.balance).toFixed(4)}`}
            </MenuButton>
          ...
          <MenuList minW={['200px', '260px']} py={0}>
            <MenuGroup title="Connected with MetaMask">
              <Box m={4} border={styleConstants.altBorder} p={4} rounded="md">
                ...
                <HStack>
                  <Heading size="md">{formatAddress(profile.ethAddress)}</Heading>
                  <Image w="20px" h="20px" src="/metamask.svg" alt="MetaMask" />
                </HStack>
              </Box>
            </MenuGroup>
            <Box borderTop={styleConstants.tableBorder} py={2} bg="gray.100">
              <MenuGroup title="Accounts across chain">
                <Box m={4} border={styleConstants.altBorder} p={4} rounded="md">
                  ...
                  <HStack>
                    <Heading size="md">
                      {`${profile.bnbBalance?.symbol} ${formatBalance(profile.bnbBalance?.balance).toFixed(4)}`}
                    </Heading>
                    <Image w="20px" h="20px" src="/metamask.svg" alt="MetaMask" />
                  </HStack>
                </Box>
